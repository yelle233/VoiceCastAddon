...
**Q: 提示"未找到法术"？**
A: 可能的原因：
1. 你没有装备该法术（法术书、卷轴或注入武器）
2. 法术正在冷却中
3. 法力值不足
4. 配置文件中有重复的触发词，识别后释放的是你没有的那个法术

请检查配置文件 `spell_aliases.json` 中是否有重复的触发词，确保唯一。

**Q: 如何跳过法术读条？**
A: 服务端管理员在 `config/voicecastaddon/server_settings.json` 中设置 `"skipCastTime": true`。语音施法将立即释放法术，说话时间可以弥补读条时间。

**Q: 如何无视法术冷却？**
A: 服务端管理员在 `config/voicecastaddon/server_settings.json` 中设置 `"ignoreCooldown": true`。语音施法将无视冷却时间。

**Q: 如何无视法力值消耗？**
A: 服务端管理员在 `config/voicecastaddon/server_settings.json` 中设置 `"ignoreManaRequirement": true`。语音施法将无视法力值剩余。
...
服务端管理员可以编辑 `config/voicecastaddon/server_settings.json` 来控制游戏平衡：

```json
{
  "skipCastTime": false,
  "ignoreCooldown": false,
  "ignoreManaRequirement": false
}
```
...
- `ignoreCooldown`: 无视法术冷却（`true` 或 `false`，默认 `false`）
  - 开启后，语音施法将无视冷却时间限制
  - 冷却期间也可以释放法术
- `ignoreManaRequirement`: 无视法力值消耗（`true` 或 `false`，默认 `false`）
  - 开启后，语音施法将无视法力值剩余
  - 法力值不足时也可以释放法术

**注意：** 这些设置由服务端控制，客户端无法修改，确保多人游戏的公平性。
